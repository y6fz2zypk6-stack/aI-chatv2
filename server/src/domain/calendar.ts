...
/**
 * 「1年7月12日(火)」形式。日付だけを見せたいとき用。
 *
 * あらすじの日付マーカーとメモリーの日付で共有する。**季節ではなく曜日を添える。**
 * 季節はロアブックのキーワードに使われることがあり、日付マーカーの側で毎行書くと
 * 意図しない発火と予算の消費につながる。会話中の感覚としても曜日のほうが効く。
 */
export function formatGameDate(cfg: CalendarConfig, time: number): string {
  const gt = toGameTime(cfg, time);
  return `${gt.year}年${gt.month}月${gt.day}日(${gt.weekday})`;
}

/**
 * 記憶の日付をプロンプト・UIに出す表記（§8.4）。「3年3月3日(火) / 3日前」の形。
 *
 * **絶対日付は常に出す。** あらすじの日付（[3年8月10日(月)]）と突き合わせられるようにするため。
