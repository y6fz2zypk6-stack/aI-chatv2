...
プロンプトへは現在時刻との差から `- (3日前) …` の形で注入する。

メモリーはチャットではなく**キャラクターに紐づく**ため、同じ世界の別チャットが
別の日付にいると差が負になったり極端に開いたりする。そこで**絶対値で保存し**、
相対表記は過去1年以内のときだけ使い、それ以外は絶対日付（`3年8月10日`）へ落とす。
一覧APIは「いまが何日か」を持たないので常に絶対日付のラベルを返す。
...
/chats/:id                 会話（ステートバーの時計から「場面を進める」）
...
**最古のスキーマのDBを実際に作り**、マイグレーション1〜6を通して結果を検証する。
...
| メモリーの日付 | 実時間の `created_at` のみ | ＋ゲーム内時刻 `game_time`、注入時は「3日前」 |
