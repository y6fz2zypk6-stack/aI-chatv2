...
| 節 | 見出し | 中身 | 出さない条件 |
|---|---|---|---|
| 画風 | **なし** | `settings.image_style_prompt` | 空のとき |
| 人物 | `Characters:` | `名前: appearance`（在席キャラ＋`include_persona` ならペルソナ） | 1行も無いとき |
| 場面 | `Scene:` | 場所 / 季節・時刻・日照 / 天候（屋内なら天候は省く）＋**地の文**（400字まで） | — |
| 構図 | `Composition:` | 「画面に文字を描かせない」1行（`image_no_text`、既定ON） | OFFのとき |

```
anime illustration, soft lighting

Characters:
ノア: silver hair, round glasses
...
- **中身が無い節は見出しごと出さない。** 空の `Characters:` をモデルへ渡さない
- **画風にだけ見出しを付けない。** ここは `Style:` の前に置く指示も試せる
  自由記述欄として使うので、アプリ側が見出しを決め打ちしない
- 場面の行は季節と時刻を必ず含むので、`Scene:` は常に出る
...
- **先頭バイトが宣言した種別と一致すること**（PNG署名 / `FFD8` / `RIFF….WEBP`）。
  宣言だけを信じない
- デコード後 6MB 以下（クライアントは長辺1024pxへ縮めて送るが、APIを直接叩かれても止まる）
...
`fileToReferenceDataUrl`（`client/src/components.tsx`）で**長辺1024pxへ縮小、
