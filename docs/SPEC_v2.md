...
在席キャラ（＋`include_persona` ならペルソナ）の**アバターを `input_references` として送る**。
アバターは 320px WebP の data URL なのでそのまま渡せる。

- **4件までに切る。** 16件受けるモデルもあれば4件のものもあるので低い方に合わせる
- **URLはクライアントから受け取らない。** サーバがアバターから組み直す。
  任意のURLを送り込めるようにすると、取得先を指定できる踏み台になる
- 送る相手はプレビューで名前を見せる（何が渡るのか分かるように）
