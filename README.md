...
コピーで完了します。**コピー・移動するときはサーバを止めて、`app.sqlite-wal` / `app.sqlite-shm` も
一緒に扱ってください**（WALモードなので、本体だけだと直近の書き込みが欠けます）。
起動ログの `[db] 使用中: …` にどのDBを開いたかと件数が出ます。スキーマは起動時に自動マイグレーションされます。
...
初回起動時にDBが空の場合、「桜坂学園の世界」（場所20件 + 既定の暦・天候テーブル）が
